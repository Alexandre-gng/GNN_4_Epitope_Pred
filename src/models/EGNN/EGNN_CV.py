"""
Cross-Validation for EGNN model.

This module provides a function to perform k-fold cross-validation on the EGNN model,
saving each fold's model state and computing Out-Of-Fold (OOF) metrics.

Adapted from GCN_CV.py — adapted for the EGNN (dict-based graph data with coordinates).
"""
...
def cross_validate_egnn(
    ...
    """
    Perform k-fold cross-validation on the EGNN model and save OOF predictions and metrics.
    ...
    Args:
        train_data: List of dict-based graph objects for training.
        val_data: List of dict-based graph objects for validation.
        blind_test_data: List of dict-based graph objects for blind testing.
        ...
        in_channels: Number of input features (embedding dimension).
                     If None, inferred from the first training sample.
    Returns:
        Dictionary containing OOF metrics and fold-wise results.
    """
    def _extract_tensor(g, keys: List[str]) -> torch.Tensor | None:
        """Robustly extract a tensor from dict/Data objects, handling legacy nested wrappers."""
        for key in keys:
            value = None
            ...
            if isinstance(g, dict):
                ...
                try:
                    if hasattr(g, "keys") and key in g.keys():
                        value = g[key]
                    else:
            ...
            if callable(value):
                ...
                if owner is not None:
                    try:
                        if hasattr(owner, "keys") and key in owner.keys():
                            nested = owner[key]
                    ...
                    nested = getattr(owner, key, None)
                    if torch.is_tensor(nested):
                        return nested

        return None

    def _sanitize_graphs(graphs: List, label: str) -> List:
        """Convert heterogeneous graph samples to canonical dict format and drop malformed ones."""
        cleaned = []
        bad = []
        for idx, g in enumerate(graphs):
            node_attrs = _extract_tensor(g, ["node_attrs", "x"])
            coords = _extract_tensor(g, ["coords", "pos"])
            edge_index = _extract_tensor(g, ["edge_index"])
            edge_attr = _extract_tensor(g, ["edge_attr"])
            y = _extract_tensor(g, ["y"])
            mask = _extract_tensor(g, ["mask"])
            mask_mode = None
            if mask is not None:
                mask_mode = "exclude"
            else:
                train_mask = _extract_tensor(g, ["train_mask"])
                if train_mask is not None:
                    mask = train_mask
                    mask_mode = "include"

            if not (torch.is_tensor(node_attrs) and torch.is_tensor(coords) and torch.is_tensor(edge_index) and torch.is_tensor(y)):
                bad.append((idx, f"missing/invalid tensors: node_attrs={type(node_attrs)}, coords={type(coords)}, edge_index={type(edge_index)}, y={type(y)}"))
                continue
        ...
        if bad:
            print(f"[WARN] Dropped {len(bad)} malformed graphs in {label}; first 3: {bad[:3]}")
        return cleaned

    # Sanitize incoming splits to guard against corrupted entries
    ...
    if len(train_data) == 0:
        raise ValueError("All training graphs were malformed after sanitization.")
    # Import FocalLoss from shared location
    sys.path.insert(0, os.path.join(project_root, "src", "models"))
    from FOCAL_LOSS import FocalLoss

    # ------------------------------------------------------------------
    # STEP 0: Infer input dimensions from data if not provided
    # ------------------------------------------------------------------
    if in_channels is None:
        in_channels = train_data[0]["node_attrs"].shape[1]

    edge_dim_from_data = config.edge_dim
    if train_data[0].get("edge_attr") is not None:
        edge_dim_from_data = train_data[0]["edge_attr"].shape[1]
    ...
    print(f"Input feature dim: {in_channels}, Edge attr dim: {edge_dim_from_data}")
    print(f"{'='*80}\n")
    ...
    for fold_idx, (train_indices, val_indices) in enumerate(kfold.split(range(num_samples))):
        ...
        # Deep-copy to avoid coordinate mutation across folds
        train_fold = [copy.deepcopy(combined_data[i]) for i in train_indices]
        val_fold = [combined_data[i] for i in val_indices]

        # EGNN uses plain lists as dataloaders (one graph per iteration)
        train_loader = train_fold
        val_loader = val_fold
        blind_test_loader = blind_test_data
        ...
        # ---- Train ----
        best_model_state, best_val_loss = train_n_epochs(
            model=model,
            train_loader=train_loader,
            val_loader=val_loader,
            test_loader=blind_test_loader,
            optimizer=optimizer,
            criterion=criterion,
            device=device,
            ...
            max_coord_step=config.max_coord_step,
            max_abs_coord_value=config.max_abs_coord_value,
        )
        ...
        verify_val_metrics = evaluate(model, val_loader, criterion, device, use_mask=use_mask)
        verify_test_metrics = evaluate(model, blind_test_loader, criterion, device, use_mask=use_mask)
        verify_val_ap = verify_val_metrics["auc_pr"]
        verify_val_roc = verify_val_metrics["auc_roc"]
        verify_test_ap = verify_test_metrics["auc_pr"]
        verify_test_roc = verify_test_metrics["auc_roc"]
        ...
        # Apply sigmoid since compute_predictions returns raw logits
        oof_probs = 1.0 / (1.0 + np.exp(-oof_preds.squeeze()))
        all_oof_preds.append(oof_probs)
